"""Generate the approved Porch Residence and its final-review package."""
...
ASSET_ID = "porch-residence"
ASSET_NAME = "Porch Residence"
FOOTPRINT = (24.0, 17.0)
...
def add_galleries_columns_and_stairs(asset, materials) -> None:
    ...
    for x in column_positions:
        ...
        add_box(asset, "ColumnBase", (x, column_y, 1.31), (0.72, 0.72, 0.16), trim, 0.025)
        add_torus(asset, "ColumnLowerMoulding", (x, column_y, 1.43), 0.43, 0.075, trim, major_segments=28)
        add_fluted_column(asset, "FlutedColumnShaft", (x, column_y, 4.86), 0.405, 6.78, trim, 20)
        add_torus(asset, "ColumnCapitalNecking", (x, column_y, 8.27), 0.42, 0.07, trim, major_segments=28)
        add_lathe(
            asset,
            "ColumnCapitalEchinus",
            (x, column_y, 8.31),
            ((0.42, 0), (0.46, 0.08), (0.53, 0.18), (0.6, 0.28)),
            trim,
            32,
        )
        add_box(asset, "ColumnCapitalAbacus", (x, column_y, 8.68), (1.24, 1.05, 0.2), trim, 0.03)
...
def add_review_environment(review) -> None:
    ...
    sun_data = bpy.data.lights.new("WarmReviewSun", "SUN")
    sun_data.energy = 2.65
    sun_data.angle = math.radians(4.5)
    sun = bpy.data.objects.new("WarmReviewSun", sun_data)
...
| Columned porch | Formal entrance | Window depth and shutters |
| --- | --- | --- |
| ![Columned porch](review/detail-portico.png) | ![Formal entrance](review/detail-entrance.png) | ![Window](review/detail-window.png) |
...
def main() -> None:
    ...
    root = bpy.data.objects.new("PorchResidence", None)
